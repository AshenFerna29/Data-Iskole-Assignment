...
def get_total_counts(superheroes_df, links_df):
    try:
        ...
        logger.error(f"Error counting totals: {e}")
        return 0, 0
...
def get_recent_heroes(superheroes_df, days=3):
    try:
        ...
        logger.error(f"Error filtering recent heroes: {e}")
...
def get_most_connected(links_df, superheroes_df, top_n=3):
    try:
        ...
        logger.error(f"Error calculating most connected superheroes: {e}")
        return []
...
def get_dataiskole_info(superheroes_df, links_df):
    try:
        ...
        logger.error(f"Error retrieving info for 'dataiskole': {e}")
        return None, []
